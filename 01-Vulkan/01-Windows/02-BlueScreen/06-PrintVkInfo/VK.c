...
VkResult TsGetPhysicalDevice(void)
{
	...
	if (VK_TRUE == bFound)
	{
		...
		{
			fprintf(gpTsFile, "[ERROR] TsGetPhysicalDevice() -> is failed to obtain grahics enabled physical device at %d\n", __LINE__);
			free(vkPhysicalDevice_array);
			vkPhysicalDevice_array = NULL;
			fprintf(gpTsFile, "[ERROR] TsGetPhysicalDevice() -> Succeeded to free vkPhysicalDevice_array at %d\n", __LINE__);
			vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
			return(vkTsResult);
		}
	}
