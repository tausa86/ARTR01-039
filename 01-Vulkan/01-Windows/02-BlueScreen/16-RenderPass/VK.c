...
// [STEP-13] Swapchain images and image views
uint32_t swapchainImageCount = UINT32_MAX;
VkImage* swapchainImage_array = NULL;
VkImageView* swapchainImageView_array = NULL;

// [STEP-14] Command Pool
VkCommandPool vkCommandPool = VK_NULL_HANDLE;

// [STEP-15] Command Buffers
...
VkResult TsInitialize(void)
{
	...
	// [STEP-13] Create Vulkan Image and Image Views
	...
	{
		fprintf(gpTsFile, "[INFO] TsInitialize() -> TsCreateImagesAndImageViews() succeeded at %d \n", __LINE__);
	}

	// [STEP-14] Create Vulkan Command Pool
	vkTsResult = TsCreateCommandPool();
	if (VK_SUCCESS != vkTsResult)
	{
		// Print actual returned VkResult value
	...
	// [STEP-14] Create Vulkan Command Buffers
	...
	// [STEP-16] Create Vulkan Render Pass
...
void TsUninitialize(void)
{
	...
	if (vkDevice)
	{
		...
		for(uint32_t i = 0; i < swapchainImageCount; i++)
		{
			...
			fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkFreeCommandBuffers() is done for iteration %d\n", i);
		}
		...
		if(vkCommandBuffer_array)
		{
			...
			fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_array is released\n");
		}
		...
		if(vkCommandPool)
		{
			...
			fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandPool() is destroyed successfully\n");
		}
		...
		for(uint32_t i = 0; i < swapchainImageCount; i++)
		{
			...
			fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkDestroyImageView() is done for iteration %d\n", i);
		}

		// Free swpachain image view array
		if(swapchainImageView_array)
		{
			free(swapchainImageView_array);
			swapchainImageView_array = NULL;
			fprintf(gpTsFile, "[INFO] TsUninitialize() -> swapchainImageView_array is released\n");
		}
...
VkResult TsCreateImagesAndImageViews(void)
{
	...
	// code
	// Step 1 Get swap chain image count
	vkTsResult = vkGetSwapchainImagesKHR(vkDevice, vkSwapchainKHR, &swapchainImageCount, NULL);
	if (VK_SUCCESS != vkTsResult)
	{
		fprintf(gpTsFile, "[ERROR] TsCreateImagesAndImageViews() -> First call to vkGetSwapchainImagesKHR() failed with %d at %d\n", vkTsResult, __LINE__);
		return(vkTsResult);
	}
	else if (0 == swapchainImageCount)
	{
		fprintf(gpTsFile, "[ERROR] TsCreateImagesAndImageViews() -> First call to vkGetSwapchainImagesKHR() resulted in swapchainImageCount to zero at %d\n", __LINE__);
		vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
		return(vkTsResult);
	}
	else
	{
		fprintf(gpTsFile, "[INFO] TsCreateImagesAndImageViews() -> First call to vkGetSwapchainImagesKHR() succeeded at %d \n", __LINE__);
	}
	...
	// Error checking
	if(NULL == swapchainImage_array)
	{
		fprintf(gpTsFile, "[ERROR] TsCreateImagesAndImageViews() -> failed to allocate memory to swapchainImage_array at  %d\n", __LINE__);
		vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
		return(vkTsResult);
	}

	// Step 3 fill Swapchain Image Array by swapchain images
	vkTsResult = vkGetSwapchainImagesKHR(vkDevice, vkSwapchainKHR, &swapchainImageCount, swapchainImage_array);
	if (VK_SUCCESS != vkTsResult)
	{
		fprintf(gpTsFile, "[ERROR] TsCreateImagesAndImageViews() -> Second call to vkGetSwapchainImagesKHR() failed with %d at %d\n", vkTsResult, __LINE__);
		return(vkTsResult);
	}
	else
	{
		fprintf(gpTsFile, "[INFO] TsCreateImagesAndImageViews() -> Second call to vkGetSwapchainImagesKHR() succeeded at %d \n", __LINE__);
	}
	...
	if(NULL == swapchainImage_array)
	{
		fprintf(gpTsFile, "[ERROR] TsCreateImagesAndImageViews() -> failed to allocate memory to swapchainImageView_array at  %d\n", __LINE__);
		vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
		return(vkTsResult);
	}

	// Step 5 Initialize VkImageCreateInfo structure
	...
	vkImageViewCreateInfo.format = VK_FORMAT_B8G8R8A8_UNORM; // VK_FORMAT_COLOR
	...
	vkImageViewCreateInfo.subresourceRange.layerCount = 1; // Mahit nahi mala atleast 1 asayla hawe
	...
	for(uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		if (VK_SUCCESS != vkTsResult)
		{
			fprintf(gpTsFile, "[ERROR] TsCreateImagesAndImageViews() -> vkCreateImageView() failed with %d for iteration %d at %d\n", vkTsResult, i, __LINE__);
			return(vkTsResult);
		}
		else
		{
			fprintf(gpTsFile, "[INFO] TsCreateImagesAndImageViews() -> vkCreateImageView() succeeded for iteration %d at %d \n", i, __LINE__);
		}
	}
...
VkResult TsCreateCommandPool(void)
{
	...
	// Code
	// Step 1 Create Command Pool Create Info Structure
	...
	if (VK_SUCCESS != vkTsResult)
	{
		fprintf(gpTsFile, "[ERROR] TsCreateCommandPool() -> vkCreateCommandPool() failed with %d at %d\n", vkTsResult, __LINE__);
		return(vkTsResult);
	}
	else
	{
		fprintf(gpTsFile, "[INFO] TsCreateCommandPool() -> vkCreateCommandPool() succeeded at %d \n", __LINE__);
	}
...
VkResult TsCreateCommandBuffers(void)
{
	...
	// Code
	// Step 1 Create and initialize Vulkan Command Buffer Allocator
	VkCommandBufferAllocateInfo vkCommandBufferAllocateInfo;
	memset((void *)&vkCommandBufferAllocateInfo, 0, sizeof(VkCommandBufferAllocateInfo));

	vkCommandBufferAllocateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
	vkCommandBufferAllocateInfo.pNext = NULL;
	// Do not have flags member
	...
	// Step 2 Allocate memory to command buffer array
	vkCommandBuffer_array = (VkCommandBuffer *)malloc(sizeof(VkCommandBuffer) * swapchainImageCount);
	// Error checking
	if(NULL == vkCommandBuffer_array)
	{
		fprintf(gpTsFile, "[ERROR] TsCreateCommandBuffers() -> failed to allocate memory to vkCommandBuffer_array at  %d\n", __LINE__);
		vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
		return(vkTsResult);
	}
	...
	for(uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		if (VK_SUCCESS != vkTsResult)
		{
			fprintf(gpTsFile, "[ERROR] TsCreateCommandBuffers() -> vkAllocateCommandBuffers() for iteration %d failed with %d at %d\n", i, vkTsResult, __LINE__);
			return(vkTsResult);
		}
		else
		{
