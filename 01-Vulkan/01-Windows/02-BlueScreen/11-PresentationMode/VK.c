...
VkResult TsInitialize(void)
{
	...
	if (VK_SUCCESS != vkTsResult)
	{
		fprintf(gpTsFile, "[ERROR] TsInitialize() -> TsGetPhysicalDeviceSurfaceFormatAndColorSpace() failed with %d at %d\n", vkTsResult, __LINE__);
		return(vkTsResult);
	}
	else
	{
		fprintf(gpTsFile, "[INFO] TsInitialize() -> TsGetPhysicalDeviceSurfaceFormatAndColorSpace() succeeded at %d \n", __LINE__);
	}
	...
	if (VK_SUCCESS != vkTsResult)
	{
		fprintf(gpTsFile, "[ERROR] TsInitialize() -> TsGetPhysicalDevicePresentMode() failed with %d at %d\n", vkTsResult, __LINE__);
		return(vkTsResult);
	}
	else
	{
		fprintf(gpTsFile, "[INFO] TsInitialize() -> TsGetPhysicalDevicePresentMode() succeeded at %d \n", __LINE__);
	}
