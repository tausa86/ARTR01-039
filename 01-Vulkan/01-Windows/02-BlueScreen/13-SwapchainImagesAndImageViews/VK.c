...
VkResult TsCreateImagesAndImageViews(void)
{
	...
	for(uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		if (VK_SUCCESS != vkTsResult)
		{
			fprintf(gpTsFile, "[ERROR] TsCreateImagesAndImageViews() -> vkCreateImageView() failed with %d at %d\n", vkTsResult, __LINE__);
			return(vkTsResult);
		}
		else
		{
			fprintf(gpTsFile, "[INFO] TsCreateImagesAndImageViews() -> vkCreateImageView() succeeded at %d \n", __LINE__);
		}
	}
