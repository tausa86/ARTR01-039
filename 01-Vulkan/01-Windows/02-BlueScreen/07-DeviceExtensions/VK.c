...
VkResult TsInitialize(void)
{
	...
	if (VK_SUCCESS != vkTsResult)
	{
		fprintf(gpTsFile, "[ERROR] TsInitialize() -> TsFillDeviceExtensionNames() failed at %d\n", __LINE__);
		return(vkTsResult);
	}
	else
	{
		fprintf(gpTsFile, "[INFO] TsInitialize() -> TsFillDeviceExtensionNames() succeeded at %d\n", __LINE__);
	}
