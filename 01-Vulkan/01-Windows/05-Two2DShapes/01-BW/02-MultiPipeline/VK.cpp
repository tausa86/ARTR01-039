...
VkPipeline vkPipeline_triangle = VK_NULL_HANDLE;
VkPipeline vkPipeline_rectangle = VK_NULL_HANDLE;
...
VkResult TsInitialize(void)
{
               ...
               VkResult TsCreatePipeline(VkPipeline*, VkPrimitiveTopology); // [STEP-26]              
              ...
              // Create Vulkan (Logical) device
               ...
               // TRIANGLE
                vkTsResult = TsCreatePipeline(&vkPipeline_triangle, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);
                if (VK_SUCCESS != vkTsResult)
                {
                               fprintf(gpTsFile, "[ERROR] TsInitialize() -> Triangle TsCreatePipeline() failed with %d at %d\n", vkTsResult, __LINE__);
                               return(vkTsResult);
                }
                else
                {
                               fprintf(gpTsFile, "[INFO] TsInitialize() -> Triangle TsCreatePipeline() succeeded at %d \n", __LINE__);
                }

                // RECTANGLE
                vkTsResult = TsCreatePipeline(&vkPipeline_rectangle, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);
                if (VK_SUCCESS != vkTsResult)
                {
                               fprintf(gpTsFile, "[ERROR] TsInitialize() -> RECTANGLE TsCreatePipelineLayout() failed with %d at %d\n", vkTsResult, __LINE__);
                               return(vkTsResult);
                }
                else
                {
                               fprintf(gpTsFile, "[INFO] TsInitialize() -> RECTANGLE TsCreatePipelineLayout() succeeded at %d \n", __LINE__);
                }
...
VkResult TsResize(int iTsWidth, int iTsHeight)
{
    ...
    if (vkDevice)
    {
        ...
        fprintf(gpTsFile, "[INFO] TsResize() -> vkPipeline_rectangle vkDestroyPipeline() is successfully done\n");
        }
        ...
        fprintf(gpTsFile, "[INFO] TsResize() -> vkPipeline_triangle vkDestroyPipeline() is successfully done\n");
        }
        ...
        if (VK_SUCCESS != vkTsResult)
        {
            fprintf(gpTsFile, "[ERROR] TsResize() -> TRIANGLE TsCreatePipeline() failed with %d at %d\n", vkTsResult, __LINE__);
            return(vkTsResult);
        }
        ...
        if (VK_SUCCESS != vkTsResult)
        {
            fprintf(gpTsFile, "[ERROR] TsResize() -> RECTANGLE TsCreatePipeline() failed with %d at %d\n", vkTsResult, __LINE__);
            return(vkTsResult);
        }
    ...
    // No need to Destroy/uninitialize Vulkan Device Queue
               ...
               if (vkDevice)
               {
                              ...
                              if(vkPipeline_rectangle)
                              {
                                vkDestroyPipeline(vkDevice, vkPipeline_rectangle, NULL);
                                vkPipeline_rectangle = VK_NULL_HANDLE;
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkPipeline_rectangle vkDestroyPipeline() is successfully done\n");
                              }
                              ...
                              if(vkPipeline_triangle)
                              {
                                vkDestroyPipeline(vkDevice, vkPipeline_triangle, NULL);
                                vkPipeline_triangle = VK_NULL_HANDLE;
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkPipeline_triangle vkDestroyPipeline() is successfully done\n");
                              }
...
VkResult TsCreatePipeline(VkPipeline* pPipeline, VkPrimitiveTopology topology)
{
    // Local variable declaration
    VkResult vkTsResult = VK_SUCCESS;
