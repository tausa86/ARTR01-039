...
void TsUninitialize(void)
{
               ...
               if (vkDevice)
               {
                              ...
                              // [STEP-34] Free Vertexdata Buffer
                              ...
                              if(vertexData_color.vkDeviceMemory)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkFreeMemory() for vertexData_color is successfully done\n");
                              }
                              ...
                              if(vertexData_color.vkBuffer)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkDestroyBuffer() for vertexData_color is successfully done\n");
                              }
...
float cubeTexcoords[] =
    ...
    // Step 6
    vkTsResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_color.vkBuffer);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkCreateBuffer() for vertexData_color failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkCreateBuffer() for vertexData_color succeeded at %d \n", __LINE__);
    }
    ...
    // Step 9
    vkTsResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_color.vkDeviceMemory);

    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkAllocateMemory() for vertexData_color failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkAllocateMemory() for vertexData_color succeeded at %d \n", __LINE__);
    }

 

    // Step 10 it binds vulkan device buffer object handle with vulkan device memory object handle
    vkTsResult = vkBindBufferMemory(vkDevice, vertexData_color.vkBuffer, vertexData_color.vkDeviceMemory, 0);

    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkBindBufferMemory() for vertexData_color failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {

        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkBindBufferMemory() for vertexData_color succeeded at %d \n", __LINE__);
    ...
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkMapMemory() for vertexData_color failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkMapMemory() for vertexData_color succeeded at %d \n", __LINE__);
    }
    ...
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkBindBufferMemory() cubeNormals failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {

        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkBindBufferMemory() cubeNormals succeeded at %d \n", __LINE__);
    } 
    ...
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkMapMemory() cubeNormals failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkMapMemory() cubeNormals succeeded at %d \n", __LINE__);
    }
...
VkResult TsCreatePipeline(void)
{
    ...
    VkVertexInputBindingDescription vkVertexInputBindingDescription_array[4];   // 1 means position if 2 then position, color etc.
    ...
    vkVertexInputBindingDescription_array[2].stride = sizeof(float) * 3;    // take step in 3 or 12 bytes R32, G32, B32
    vkVertexInputBindingDescription_array[2].inputRate = VK_VERTEX_INPUT_RATE_VERTEX;   // This is vertices rate i.e. kimmat

    // For texcoord
    vkVertexInputBindingDescription_array[3].binding = 3;   // layout (location = 3) in vertex shader
    vkVertexInputBindingDescription_array[3].stride = sizeof(float) * 2;    // take step in 2 or 8 bytes
    vkVertexInputBindingDescription_array[3].inputRate = VK_VERTEX_INPUT_RATE_VERTEX;   // This is vertices rate i.e. kimmat

    // Shader madhle attributes
    VkVertexInputAttributeDescription vkVertexInputAttributeDescription_array[4];  // 1 means position if 2 then position, color etc.
    ...
    // For normal
    vkVertexInputAttributeDescription_array[2].binding = 2;     // vkVertexInputBindingDescription_array[2].binding = 2 <-> vkVertexInputAttributeDescription_array[0].binding = 0; but this will change in interleave
    vkVertexInputAttributeDescription_array[2].location = 2;    // layout(location = 2) in vec4 vPosition; // in Shader.vert
    vkVertexInputAttributeDescription_array[2].format = VK_FORMAT_R32G32B32_SFLOAT;     // Majha vertex X Y Z aahe te Signed Float 32-bit R32 = X, G32 = Y, G32 = Z
    vkVertexInputAttributeDescription_array[2].offset = 0;      // In interleaved how to jump in attribute layout. For now it is 0

    // For texcoord
    vkVertexInputAttributeDescription_array[3].binding = 3;     // vkVertexInputBindingDescription_array[3].binding = 3 <-> vkVertexInputAttributeDescription_array[1].binding = 1; but this will change in interleave
    vkVertexInputAttributeDescription_array[3].location = 3;    // layout(location = 3) in vec4 vTexCoord; // in Shader.vert
    vkVertexInputAttributeDescription_array[3].format = VK_FORMAT_R32G32_SFLOAT;     // Majha vertex S and T aahe te Signed Float 32-bit R32 = S, G32 = T
    vkVertexInputAttributeDescription_array[3].offset = 0;      // In interleaved how to jump in attribute layout. For now it is 0
...
VkResult TsBuildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 
            1, // Binding Point
            1, // Number of buffer
            &vertexData_color.vkBuffer, // Which buffer
        ...
        vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 2, 1, &vertexData_normal.vkBuffer, vkDeviceSize_offset_normals);
