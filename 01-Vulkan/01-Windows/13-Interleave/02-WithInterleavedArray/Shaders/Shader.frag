...
layout(location = 2) in vec3 outTransformedNormal;
layout(location = 3) in vec3 outLightDirection;
layout(location = 4) in vec3 outViewerVector;
...
void main(void)
{
    ...
    FragColor = outColor * texture(uTextureSampler, outTexCoords) * vec4(fongADS_light, 1.0);
}
