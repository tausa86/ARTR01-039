...
void TsUninitialize(void)
{
               ...
               if (vkDevice)
               {
                              ...
                              // [STEP-22]
                              if(vertexData_pcnt.vkDeviceMemory)
                              {
                                vkFreeMemory(vkDevice, vertexData_pcnt.vkDeviceMemory, NULL);
                                vertexData_pcnt.vkDeviceMemory = VK_NULL_HANDLE;
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkFreeMemory() is successfully done\n");
                              }

                              if(vertexData_pcnt.vkBuffer)
                              {
                                vkDestroyBuffer(vkDevice, vertexData_pcnt.vkBuffer, NULL);
                                vertexData_pcnt.vkBuffer = VK_NULL_HANDLE;
...
VkResult TsCreateVertexBuffer(void)
{
    ...
    // Step 4
    memset((void*)&vertexData_pcnt, 0, sizeof(VertexData));
    ...
    vkBufferCreateInfo.size = sizeof(cube_PCNT);
    ...
    // Step 6
    vkTsResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_pcnt.vkBuffer);
    if (VK_SUCCESS != vkTsResult)
    {
    ...
    VkMemoryRequirements vkMemoryRequirements;
    memset((void*)&vkMemoryRequirements, 0, sizeof(VkMemoryRequirements));
    vkGetBufferMemoryRequirements(vkDevice, vertexData_pcnt.vkBuffer, &vkMemoryRequirements);
    // No error checking
    // Step 8
    VkMemoryAllocateInfo vkMemoryAllocateInfo;
    memset((void*)&vkMemoryAllocateInfo, 0, sizeof(VkMemoryAllocateInfo));
    ...
    // Step 9
    vkTsResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_pcnt.vkDeviceMemory);
    if (VK_SUCCESS != vkTsResult)
    {
    ...
    vkTsResult = vkBindBufferMemory(vkDevice, vertexData_pcnt.vkBuffer, vertexData_pcnt.vkDeviceMemory, 0);
    if (VK_SUCCESS != vkTsResult)
    {
    ...
    vkTsResult = vkMapMemory(
        vkDevice,
        vertexData_pcnt.vkDeviceMemory, //
        0, // start
    ...
    // Step 12
    memcpy(data, cube_PCNT, sizeof(cube_PCNT));

    // Step 13
    vkUnmapMemory(vkDevice, vertexData_pcnt.vkDeviceMemory);
...
VkResult TsCreateDescriptorSetLayout(void)
{
    ...
    vkDescriptorSetLayoutBinding_array[0].stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT; // As we have light and mvp in uniform
...
VkResult TsCreatePipeline(void)
{
    ...
    vkVertexInputBindingDescription_array[0].stride = sizeof(float) * 11;    // take step in 11 P3C3N3T2 or 44 bytes
    ...
    vkVertexInputAttributeDescription_array[0].offset = sizeof(float) * 0;      // In interleaved how to jump in attribute layout. For now it is 0

    // For color
    vkVertexInputAttributeDescription_array[1].binding = 0;     // vkVertexInputBindingDescription_array[1].binding = 1 <-> vkVertexInputAttributeDescription_array[1].binding = 1; but this will change in interleave
    ...
    vkVertexInputAttributeDescription_array[1].offset = sizeof(float) * 3;      // In interleaved how to jump in attribute layout. For now it is 0

    // For normal
    vkVertexInputAttributeDescription_array[2].binding = 0;     // vkVertexInputBindingDescription_array[2].binding = 2 <-> vkVertexInputAttributeDescription_array[0].binding = 0; but this will change in interleave
    ...
    vkVertexInputAttributeDescription_array[2].offset = sizeof(float) * 6;      // In interleaved how to jump in attribute layout. For now it is 0

    // For texcoord
    vkVertexInputAttributeDescription_array[3].binding = 0;     // vkVertexInputBindingDescription_array[3].binding = 3 <-> vkVertexInputAttributeDescription_array[1].binding = 1; but this will change in interleave
    ...
    vkVertexInputAttributeDescription_array[3].offset = sizeof(float) * 9;      // In interleaved how to jump in attribute layout. For now it is 0
...
VkResult TsBuildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        VkDeviceSize vkDeviceSize_offset_pcnt[1];
        memset((void*)vkDeviceSize_offset_pcnt, 0, sizeof(VkDeviceSize) * ARRAY_SIZE(vkDeviceSize_offset_pcnt));

        vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 
            0, // Binding Point
            1, // Number of buffer
            &vertexData_pcnt.vkBuffer, // Which buffer
            vkDeviceSize_offset_pcnt   
        );
