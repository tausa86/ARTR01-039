...
const uint32_t triangle_position_indices[3] = {0,1,2};  // Triangle position indices as it needs in TsDisplay() as well 
              ...
              // Destroy window
...

               {
                              ...
                              if(vertexData_position_index.vkDeviceMemory)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkFreeMemory() vertexData_position_index is successfully done\n");
                              }
                              ...
                              if(vertexData_position_index.vkBuffer)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkDestroyBuffer() vertexData_position_index is successfully done\n");
                              }
...
VkResult TsCreateVertexBuffer(void)
{
    ...
    // Step 6
    vkTsResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_position_index.vkBuffer);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkCreateBuffer() for vertexData_position_index buffer failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkCreateBuffer() for vertexData_position_index buffer succeeded at %d \n", __LINE__);
    }
    ...
    // Step 9
    vkTsResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_position_index.vkDeviceMemory);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkAllocateMemory() for vertexData_position_index failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkAllocateMemory() for vertexData_position_index succeeded at %d \n", __LINE__);
    }

    // Step 10 it binds vulkan device buffer object handle with vulkan device memory object handle
    vkTsResult = vkBindBufferMemory(vkDevice, vertexData_position_index.vkBuffer, vertexData_position_index.vkDeviceMemory, 0);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkBindBufferMemory() for vertexData_position_index failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkBindBufferMemory() for vertexData_position_index succeeded at %d \n", __LINE__);
    }
    ...
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkMapMemory() for vertexData_position_index failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkMapMemory() for vertexData_position_index succeeded at %d \n", __LINE__);
    }
...
VkResult TsBuildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        // 4th parameter correspods to 3rd parameter of glDrawElements() 
        // 2nd parameter correesponds to 4th parameter of glDrawElements()
        vkCmdBindIndexBuffer(vkCommandBuffer_array[i], vertexData_position_index.vkBuffer, 0, VK_INDEX_TYPE_UINT32);   // 4th parameter UINT32 type match kartoh traiangle position chya type unit32_t 

        // Here we should call Vulkan Drawing Functions
        // Model loading sathi lagnaar        
        unsigned int numIndices = (unsigned int)(sizeof(triangle_position_indices) / sizeof(triangle_position_indices[0])); // Array size 

        // 
        vkCmdDrawIndexed(
            vkCommandBuffer_array[i],
            numIndices, // 2nd parameter corresponds to 2nd parameter to glDrawElements()
            1,   // Geometry instances
            0,  // 0th index pasoon from Index Buffer
            0,  // Vertex offset if we wish to add any offset into starting index of vertex buffer
            1   // n-th instance (kitiwa instance)
        );
