...
// GLSL/SPIRV related header files
#include<SPIRV\GLslangToSpv.h>
#include<Public\ShaderLang.h>   // Started needed recently 1.2.8 and above SDK version
...
// GLslang\SPIRV related library and this changes as version of SDK changes
/*#pragma comment(lib, "GenericCodeGen.lib")
#pragma comment(lib, "MachineIndependent.lib")
#pragma comment(lib, "SPIRV.lib")*/
#pragma comment(lib, "SPIRV-Tools.lib") // 
#pragma comment(lib, "SPIRV-Tools-opt.lib") // Optional functions and must required
#pragma comment(lib, "glslang.lib") // Started needing recently
...
VkResult TsCreateShaders(void)
{
    ...
    // For vertex shader
    const char * vertexShaderSourceCode = "#version 450 core\n \
...
{\n \
    ...
    if(FALSE == bDone)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> glsl2spv() Failed for vertex shader at %d\n", __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
    ...
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> glsl2spv() Succeeded to create vertex shader at %d\n", __LINE__);
    }
...
{\n \
    ...
    if(FALSE == bDone)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> glsl2spv() Failed for fargement shader at %d\n", __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
    ...
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> glsl2spv() Succeeded to create fragment shader at %d\n", __LINE__);
    }
    ...
    fprintf(gpTsFile, "[INFO] TsCreateShaders() -> Fargment Shader Module is successfully created!!!\n");

    return(vkTsResult);
}

BOOL glsl2spv(const VkShaderStageFlagBits shader_type, const char * pShader, vector<unsigned int>& spv)
{
    // Function declarations
    ...
    EShLanguage language = findShaderLanguage(shader_type);

    // Create TShader Type Shader from above EShLanguage by calling TShader type constructor
    glslang::TShader shader(language);

    // Put our shader source code into a string array
    const char * shaderStrings[1];
    shaderStrings[0] = pShader;

    // Initialize built in shader resources
    ...
    // Specify rules of shader compilation as SPIRV rules and Vulkan rules
    EShMessages ruleMessages = (EShMessages)(EShMsgSpvRules | EShMsgVulkanRules);

    // Parse the TShader
    if(!shader.parse(&resources, 100, false, ruleMessages))
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> parse() failed with log message %s at %d\n", shader.getInfoLog(), __LINE__);
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> parse() failed with debug log message %s at %d\n", shader.getInfoDebugLog(), __LINE__);
        return(FALSE);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> parse() succeeded at %d\n", __LINE__);
    }

    // Add shader to TProgram 
    glslang::TProgram program;
    program.addShader(&shader); // Passing cooked shader

    // Link the program
    if(!program.link(ruleMessages))
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> link() failed with log message %s at %d\n", shader.getInfoLog(), __LINE__);
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> link() failed with debug log message %s at %d\n", shader.getInfoDebugLog(), __LINE__);
        return(FALSE);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> link() succeeded at %d\n", __LINE__);
    }

    // And now set this whole cooked program & spirv code to glslang library
    glslang::GlslangToSpv(*program.getIntermediate(language), spv);

    return(TRUE);
}

EShLanguage findShaderLanguage(const VkShaderStageFlagBits shader_type)
{
    // Code
    switch(shader_type)
    {
