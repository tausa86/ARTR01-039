    ...
    // Local function decalarations
...

               {
                              ...
                              if(vkShaderModule_geometry_shader)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkDestroyShaderModule() vkShaderModule_geometry_shader is successfully done\n");
                              }
...
VkResult TsCreateVulkanDevice(void)
{
               ...
               else
               {
                    ...
                    fprintf(gpTsFile, "[ERROR] TsCreateVulkanDevice() -> Geometry Shader not supported with %d at %d\n", vkTsResult, __LINE__);
...
VkResult TsCreateShaders(void)
{
    ...
    if(NULL == fp)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> Geom Failed to open %s at %d\n", szFileName, __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> Geom Succeeded to open %s at %d\n", szFileName, __LINE__);
    }
    ...
    if(0 == size)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> Geom The SPIRV shader file size 0 %s at %d\n", szFileName, __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkTsResult);
    }
    ...
    if(1 != retVal)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> Geom Failed to read the SPIRV shader file %s at %d\n", szFileName, __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> Geom Succeeded to read %s at %d\n", szFileName, __LINE__);
    }
    ...
    vkTsResult = vkCreateShaderModule(vkDevice, &vkShaderModuleCreateInfo, NULL, &vkShaderModule_geometry_shader);

    if(NULL == fp)

    {

        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> vkCreateShaderModule() Failed in geometry shader at %d\n", __LINE__);
...

    {
        ...
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> vkCreateShaderModule() Succeeded in geometry shader at %d\n", __LINE__);
    ...
    fprintf(gpTsFile, "[INFO] TsCreateShaders() -> Geometry Shader Module is successfully created!!!\n");
...
VkResult TsCreatePipeline(void)
{
   ...
   // Create Viewport
    ...
    VkPipelineShaderStageCreateInfo vkPipelineShaderStageCreateInfo_array[3];   // Vertex and Fragment shaders
    ...
    vkPipelineShaderStageCreateInfo_array[1].pName = "main";    // Name of entry point function of gemoetry shader
    ...
    vkPipelineShaderStageCreateInfo_array[2].pName = "main";    // Name of entry point function of fragment shader
