...
// VK_KHR_SWAPCHAIN_EXTENSION_NAME and VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSOIN_NAME
...
VkCommandBuffer * vkCommandBuffer_pos_1024x1024_graphics_array = NULL;
VkCommandBuffer * vkCommandBuffer_pos_512x512_graphics_array = NULL;
VkCommandBuffer * vkCommandBuffer_pos_256x256_graphics_array = NULL;
VkCommandBuffer * vkCommandBuffer_pos_128x128_graphics_array = NULL;
VkCommandBuffer * vkCommandBuffer_pos_64x64_graphics_array = NULL;
BOOL bMesh1024Choosen = FALSE;
BOOL bMesh512Choosen = FALSE;
BOOL bMesh256Choosen = FALSE;
BOOL bMesh128Choosen = FALSE;
BOOL bMesh64Choosen = TRUE;
char colorFromKey = 'O';
               ...
               case WM_KEYDOWN:
                              switch (wTsParam)
                              {
                                ...
                                case 49:  // '1' key
                                    ...
                                    break;
                                case 50:  // '2' key
                                if(bMesh128Choosen == FALSE)
                                    bMesh128Choosen = TRUE;
                                else
                                    bMesh128Choosen = FALSE;
                                    break;
                                case 51:  // '3' key
                                if(bMesh256Choosen == FALSE)
                                    bMesh256Choosen = TRUE;
                                else
                                    bMesh256Choosen = FALSE;
                                    break;
                                case 52:  // '4' key
                                if(bMesh512Choosen == FALSE)
                                    bMesh512Choosen = TRUE;
                                else
                                    bMesh512Choosen = FALSE;
                                    break;
                                case 53:  // '5' key
                                    if(bMesh1024Choosen == FALSE)
                                        bMesh1024Choosen = TRUE;
                                    else
                                        bMesh1024Choosen = FALSE;
                                    break;
                                case 0x46:    // case 'F'
                                case 0x66:    // case 'f'
                                    TsToggleFullScreen();
...
VkResult TsInitialize(void)
{
               ...
               VkResult TsCreateCommandBuffers(VkCommandBuffer **);
               void TsInitializeSinewaveArrays(int, int);
               VkResult TsCreateVertexBuffer(int, int, VertexData *); // [STEP-22]
              ...
              // Create Vulkan (Logical) device
               ...
               oclResult = intialize_opencl();
               if(CL_SUCCESS != oclResult)
               {
                    // Print actual returned VkResult value and hardcoded return value
                    fprintf(gpTsFile, "[ERROR] TsInitialize() -> intialize_opencl() failed with %d at %d\n", vkTsResult, __LINE__);
                    vkTsResult = VK_ERROR_INITIALIZATION_FAILED; 
                    return(vkTsResult);
               }
               else
               {
                    fprintf(gpTsFile, "[INFO] TsInitialize() -> intialize_opencl() succeeded at %d \n", __LINE__);
               }
               ...
               vkTsResult = TsCreateCommandBuffers(&vkCommandBuffer_pos_64x64_graphics_array);
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 64x64 TsCreateCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
                              fprintf(gpTsFile, "[INFO] TsInitialize() -> 64x64 TsCreateCommandBuffers() succeeded at %d \n", __LINE__);
               }

               // Initialize sinewave array for 64x64
               TsInitializeSinewaveArrays(64, 64);
 
               // [STEP-22] Create Vertex Buffer 64x64
               memset((void*)&vertex_position_64x64_graphics, 0, sizeof(VertexData));
               vkTsResult = TsCreateVertexBuffer(64, 64, &vertex_position_64x64_graphics);
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 64x64 TsCreateVertexBuffer() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
                              fprintf(gpTsFile, "[INFO] TsInitialize() -> 64x64 TsCreateVertexBuffer() succeeded at %d \n", __LINE__);
               }

               // Create external vertex buffer
               memset((void *)&vertexData_external, 0, sizeof(VertexData));
               vkTsResult = TsCreateExternalVertexBuffer(64, 64, &vertexData_external);

               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 64x64 TsCreateExternalVertexBuffer() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
                              fprintf(gpTsFile, "[INFO] TsInitialize() -> 64x64 TsCreateExternalVertexBuffer() succeeded at %d \n", __LINE__);
               }

               // 128
               // Create command buffers
               vkTsResult = TsCreateCommandBuffers(&vkCommandBuffer_pos_128x128_graphics_array);
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 128x128 TsCreateCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
                              fprintf(gpTsFile, "[INFO] TsInitialize() -> 128x128 TsCreateCommandBuffers() succeeded at %d \n", __LINE__);
               }

               // Initialize sinewave array for 128x128
               TsInitializeSinewaveArrays(128, 128);
 
               // [STEP-22] Create Vertex Buffer 128x128
               memset((void*)&vertex_position_128x128_graphics, 0, sizeof(VertexData));
               vkTsResult = TsCreateVertexBuffer(128, 128, &vertex_position_128x128_graphics);
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 128x128 TsCreateVertexBuffer() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
               ...
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 256x256 TsCreateCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
                              fprintf(gpTsFile, "[INFO] TsInitialize() -> 256x256 TsCreateCommandBuffers() succeeded at %d \n", __LINE__);
               }

               // Initialize sinewave array for 256x256
               TsInitializeSinewaveArrays(256, 256);
 
               // [STEP-22] Create Vertex Buffer 256x256
               memset((void*)&vertex_position_256x256_graphics, 0, sizeof(VertexData));
               vkTsResult = TsCreateVertexBuffer(256, 256, &vertex_position_256x256_graphics);
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 256x256 TsCreateVertexBuffer() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
               ...
               // 512
               // Create command buffers
               vkTsResult = TsCreateCommandBuffers(&vkCommandBuffer_pos_512x512_graphics_array);
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 512x512 TsCreateCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
                              fprintf(gpTsFile, "[INFO] TsInitialize() -> 512x512 TsCreateCommandBuffers() succeeded at %d \n", __LINE__);
               }

               // Initialize sinewave array for 512x512
               TsInitializeSinewaveArrays(512, 512);
 
               // [STEP-22] Create Vertex Buffer 512x512
               memset((void*)&vertex_position_512x512_graphics, 0, sizeof(VertexData));
               vkTsResult = TsCreateVertexBuffer(512, 512, &vertex_position_512x512_graphics);
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 512x512 TsCreateVertexBuffer() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
               ...
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 1024x1024 TsCreateCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
                              fprintf(gpTsFile, "[INFO] TsInitialize() -> 1024x1024 TsCreateCommandBuffers() succeeded at %d \n", __LINE__);
               }

               // Initialize sinewave array for 1024x1024
               TsInitializeSinewaveArrays(1024, 1024);
 
               // [STEP-22] Create Vertex Buffer 1024x1024
               memset((void*)&vertex_position_1024x1024_graphics, 0, sizeof(VertexData));
               vkTsResult = TsCreateVertexBuffer(1024, 1024, &vertex_position_1024x1024_graphics);
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsInitialize() -> 1024x1024 TsCreateVertexBuffer() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               else
               {
...
cl_int initialize_opencl(void)
{
    ...
    for(unsigned int i =0; i<platform_count; i++)
    {
        ...
        oclPlatform = (char *)malloc(infoSize * sizeof(char));
        // Add error checking here
    ...
    for(unsigned int i = 0; i<device_count; i++)
    {
        ...
        for(unsigned int j = 0; j < CL_UUID_SIZE_KHR; j++)
        {
...
VkResult TsResize(int iTsWidth, int iTsHeight)
{
    ...
    if (vkDevice)
    {
        ...
        for(uint32_t i = 0; i < swapchainImageCount; i++)
        {
                        vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_pos_1024x1024_graphics_array[i]);
                        //fprintf(gpTsFile, "[INFO] TsResize() -> vkFreeCommandBuffers() vkCommandBuffer_pos_1024x1024_graphics_array is successfully done for iteration: %d\n", i);
                        vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_pos_512x512_graphics_array[i]);
                        vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_pos_256x256_graphics_array[i]);
                        vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_pos_128x128_graphics_array[i]);
                        vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_pos_64x64_graphics_array[i]);
        }

        if(vkCommandBuffer_pos_1024x1024_graphics_array)
        {
                        free(vkCommandBuffer_pos_1024x1024_graphics_array);
                        vkCommandBuffer_pos_1024x1024_graphics_array = NULL;
                        fprintf(gpTsFile, "[INFO] TsResize() -> vkCommandBuffer_pos_1024x1024_graphics_array is successfully freed\n");
        }

        if(vkCommandBuffer_pos_512x512_graphics_array)
        {
                        free(vkCommandBuffer_pos_512x512_graphics_array);
                        vkCommandBuffer_pos_512x512_graphics_array = NULL;
                        fprintf(gpTsFile, "[INFO] TsResize() -> vkCommandBuffer_pos_512x512_graphics_array is successfully freed\n");
        }

        if(vkCommandBuffer_pos_256x256_graphics_array)
        {
                        free(vkCommandBuffer_pos_256x256_graphics_array);
                        vkCommandBuffer_pos_256x256_graphics_array = NULL;
                        fprintf(gpTsFile, "[INFO] TsResize() -> vkCommandBuffer_pos_256x256_graphics_array is successfully freed\n");
        }

        if(vkCommandBuffer_pos_128x128_graphics_array)
        {
                        free(vkCommandBuffer_pos_128x128_graphics_array);
                        vkCommandBuffer_pos_128x128_graphics_array = NULL;
                        fprintf(gpTsFile, "[INFO] TsResize() -> vkCommandBuffer_pos_128x128_graphics_array is successfully freed\n");
        }

        if(vkCommandBuffer_pos_64x64_graphics_array)
        {
                        free(vkCommandBuffer_pos_64x64_graphics_array);
                        vkCommandBuffer_pos_64x64_graphics_array = NULL;
                        fprintf(gpTsFile, "[INFO] TsResize() -> vkCommandBuffer_pos_64x64_graphics_array is successfully freed\n");
        }
...
// [STEP-26] Create Pipeline
        ...
        // Create Frame Buffer
               ...
               if (VK_SUCCESS != vkTsResult)
               {
                              fprintf(gpTsFile, "[ERROR] TsResize() -> 1024x1024 TsCreateCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                              return(vkTsResult);
               }
               ...
               if (VK_SUCCESS != vkTsResult)
                {
                                fprintf(gpTsFile, "[ERROR] TsResize() -> 512x512 TsCreateCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                                return(vkTsResult);
                }
                ...
                if (VK_SUCCESS != vkTsResult)
                {
                                fprintf(gpTsFile, "[ERROR] TsResize() -> 256x256 TsCreateCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                                return(vkTsResult);
                }
                ...
                if (VK_SUCCESS != vkTsResult)
                {
                                fprintf(gpTsFile, "[ERROR] TsResize() -> 128x128 TsCreateCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                                return(vkTsResult);
                }
                ...
                if (VK_SUCCESS != vkTsResult)
                {
                                fprintf(gpTsFile, "[ERROR] TsResize() -> 64x64 TsCreateCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                                return(vkTsResult);
                }
...
VkResult TsDisplay(void)
{
    ...
    {
        fprintf(gpTsFile, "[ERROR] TsDisplay() -> No mesh is chosen at %d \n", __LINE__);
    ...
    if (VK_SUCCESS != vkTsResult)
    {
                    fprintf(gpTsFile, "[ERROR] TsDisplay() -> TsBuildCommandBuffers() failed with %d at %d\n", vkTsResult, __LINE__);
                    return(vkTsResult);
    }
...

{
...

               {
                              ...
                              if(vertex_position_512x512_graphics.vkDeviceMemory)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vertex_position_512x512_graphics vkFreeMemory() is successfully done\n");
                              }
                              ...
                              if(vertex_position_512x512_graphics.vkBuffer)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vertex_position_512x512_graphics vkDestroyBuffer() is successfully done\n");
                              }  
                              ...
                              if(vertex_position_256x256_graphics.vkDeviceMemory)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vertex_position_256x256_graphics vkFreeMemory() is successfully done\n");
                              }
                              ...
                              if(vertex_position_256x256_graphics.vkBuffer)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vertex_position_256x256_graphicsc vkDestroyBuffer() is successfully done\n");
                              }  
                              ...
                              if(vertex_position_128x128_graphics.vkDeviceMemory)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vertex_position_128x128_graphics vkFreeMemory() is successfully done\n");
                              }
                              ...
                              if(vertex_position_128x128_graphics.vkBuffer)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vertex_position_128x128_graphics vkDestroyBuffer() is successfully done\n");
                              } 
                              ...
                              if(vertex_position_64x64_graphics.vkDeviceMemory)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vertex_position_64x64_graphics vkFreeMemory() is successfully done\n");
                              }
                              ...
                              if(vertex_position_64x64_graphics.vkBuffer)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vertex_position_64x64_graphics vkDestroyBuffer() is successfully done\n");
                              } 

                              if(vertex_position_1024x1024_graphics.vkDeviceMemory)
                              {
                                vkFreeMemory(vkDevice, vertex_position_1024x1024_graphics.vkDeviceMemory, NULL);
                                vertex_position_1024x1024_graphics.vkDeviceMemory = VK_NULL_HANDLE;
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vertex_position_1024x1024_graphics vkFreeMemory() is successfully done\n");
                              }

                              if(vertex_position_1024x1024_graphics.vkBuffer)
                              {
                                vkDestroyBuffer(vkDevice, vertex_position_1024x1024_graphics.vkBuffer, NULL);
                                vertex_position_1024x1024_graphics.vkBuffer = VK_NULL_HANDLE;
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vertex_position_1024x1024_graphics vkDestroyBuffer() is successfully done\n");
                              } 
                              ...
                              if(vkDescriptorPool)
                             {
                              ...
                              {
                                             vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_pos_1024x1024_graphics_array[i]);
                                             fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_pos_1024x1024_graphics_array vkFreeCommandBuffers() is successfully done for iteration: %d\n", i);
                                             vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_pos_512x512_graphics_array[i]);
                                             fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_pos_512x512_graphics_array vkFreeCommandBuffers() is successfully done for iteration: %d\n", i);
                                             vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_pos_256x256_graphics_array[i]);
                                             fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_pos_256x256_graphics_array vkFreeCommandBuffers() is successfully done for iteration: %d\n", i);
                                             vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_pos_128x128_graphics_array[i]);
                                             fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_pos_128x128_graphics_array vkFreeCommandBuffers() is successfully done for iteration: %d\n", i);
                                             vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer_pos_64x64_graphics_array[i]);
                                             fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_pos_64x64_graphics_array vkFreeCommandBuffers() is successfully done for iteration: %d\n", i);   
                              }
                              ...
                              if(vkCommandBuffer_pos_1024x1024_graphics_array)
                              {
                                             free(vkCommandBuffer_pos_1024x1024_graphics_array);
                                             vkCommandBuffer_pos_1024x1024_graphics_array = NULL;
                                             fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_pos_1024x1024_graphics_array is successfully freed\n");
                              }
                              ...
                              if(vkCommandBuffer_pos_512x512_graphics_array)
                              {
                                             free(vkCommandBuffer_pos_512x512_graphics_array);
                                             vkCommandBuffer_pos_512x512_graphics_array = NULL;
                                             fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_pos_512x512_graphics_array is successfully freed\n");
                              }
                              ...
                              if(vkCommandBuffer_pos_256x256_graphics_array)
                              {
                                             free(vkCommandBuffer_pos_256x256_graphics_array);
                                             vkCommandBuffer_pos_256x256_graphics_array = NULL;
                                             fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_pos_256x256_graphics_array is successfully freed\n");
                              }
                              ...
                              if(vkCommandBuffer_pos_128x128_graphics_array)
                              {
                                             free(vkCommandBuffer_pos_128x128_graphics_array);
                                             vkCommandBuffer_pos_128x128_graphics_array = NULL;
                                             fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_pos_128x128_graphics_array is successfully freed\n");
                              }
                                ...
                                if(vkCommandBuffer_pos_64x64_graphics_array)
                                {
                                                 free(vkCommandBuffer_pos_64x64_graphics_array);
                                                 vkCommandBuffer_pos_64x64_graphics_array = NULL;
                                                 fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkCommandBuffer_pos_64x64_graphics_array is successfully freed\n");
                                }
...
VkResult TsFillDeviceExtensionNames(void)
{
               ...
               if (VK_FALSE == vulkanExternalMemoryWin32ExtensionFound)
               {
                    vkTsResult = VK_ERROR_INITIALIZATION_FAILED; // Return hardcoded failure
                    fprintf(gpTsFile, "[ERROR] TsFillDeviceExtensionNames() -> VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME not found at %d\n", __LINE__);
                    return(vkTsResult);
               }
               else
               {
                    fprintf(gpTsFile, "[INFO] TsFillDeviceExtensionNames() -> VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME found at %d\n", __LINE__);
               }
...
VkResult TsCreateCommandPool(void)
{
               ...
               else
               {
                ...
                // Command pool is created successfully
               return(vkTsResult);
}

 

VkResult TsCreateCommandBuffers(VkCommandBuffer ** ppVkCommandBuffer_array)
{
               // Local variable declaration
               VkResult vkTsResult = VK_SUCCESS;
              VkCommandBuffer* vkCommandBuffer_array = NULL;
...
VkResult TsCreateVertexBuffer(int meshWidth, int meshHeight, VertexData * pVertexData)
{
    // Local variable decalration
    VkResult vkTsResult = VK_SUCCESS;
...
void TsInitializeSinewaveArrays(int meshWidth, int meshHeight)
{
    // Local variable declarations
...
VkResult TsUpdateUnifomBuffer(void)
{
    ...
    myuniformData.modelMatrix = glm::translate(
        ...
        glm::vec3(0.0f, 0.0f, -1.0f)    // Translation Vector x, y, z
    );
...
VkResult TsCreatePipeline(void)
{
    ...
    vkVertexInputBindingDescription_array[0].stride = sizeof(float) * 4;    // Because in shader all 4's are there x,y,z,w
    ...
    vkVertexInputAttributeDescription_array[0].format = VK_FORMAT_R32G32B32A32_SFLOAT;     // Majha vertex X Y Z aahe te Signed Float 32-bit R32 = X, G32 = Y, G32 = Z
...
VkResult TsBuildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        if(TRUE == bMesh1024Choosen)
        {
            vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertex_position_1024x1024_graphics.vkBuffer, vkDeviceSize_offset_array);
        ...
        {
            vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertex_position_512x512_graphics.vkBuffer, vkDeviceSize_offset_array);
            vkCmdDraw(vkCommandBuffer_array[i],
                512 * 512,  // Kiti vertices draw karaychet
                1, // Kiti instances aahet 1
                0, // Drawing koothoon start karaych
                0  // From 1st instance
            );
        ...
        {
            vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertex_position_256x256_graphics.vkBuffer, vkDeviceSize_offset_array);
            vkCmdDraw(vkCommandBuffer_array[i],
                256 * 256,  // Kiti vertices draw karaychet
                1, // Kiti instances aahet 1
                0, // Drawing koothoon start karaych
                0  // From 1st instance
            );
        ...
        {
            vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertex_position_128x128_graphics.vkBuffer, vkDeviceSize_offset_array);
            vkCmdDraw(vkCommandBuffer_array[i],
                128 * 128,  // Kiti vertices draw karaychet
                1, // Kiti instances aahet 1
                0, // Drawing koothoon start karaych
                0  // From 1st instance
            );
        ...
        {
            vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertex_position_64x64_graphics.vkBuffer, vkDeviceSize_offset_array);
            vkCmdDraw(vkCommandBuffer_array[i],
                64 * 64,  // Kiti vertices draw karaychet
                1, // Kiti instances aahet 1
                0, // Drawing koothoon start karaych
                0  // From 1st instance
            );
 ...
 VkResult prepareSinewaveForCPU(unsigned int meshWidth, unsigned int meshHeight, float animTime)
 {
    // Function
    ...
    // fill sinewave arrays
    fillSinewaveArraysForCPU(meshWidth, meshHeight, animTime);
    ...
    if(meshWidth == 1024 && meshHeight == 1024)
    {
        ...
        {
            fprintf(gpTsFile, "[ERROR] prepareSinewaveForCPU() -> vkMapMemory() failed\n");
            return(vkResult);
        }
    ...
    {
        vkResult = vkMapMemory(vkDevice, vertex_position_512x512_graphics.vkDeviceMemory, 0, size, 0, &data);
        if(vkResult != VK_SUCCESS)
        {
            fprintf(gpTsFile, "[ERROR] prepareSinewaveForCPU() -> vkMapMemory() failed\n");
            return(vkResult);
    ...
    {
        vkResult = vkMapMemory(vkDevice, vertex_position_256x256_graphics.vkDeviceMemory, 0, size, 0, &data);
        if(vkResult != VK_SUCCESS)
        {
            fprintf(gpTsFile, "[ERROR] prepareSinewaveForCPU() -> vkMapMemory() failed\n");
            return(vkResult);
    ...
    {
        vkResult = vkMapMemory(vkDevice, vertex_position_128x128_graphics.vkDeviceMemory, 0, size, 0, &data);
        if(vkResult != VK_SUCCESS)
        {
            fprintf(gpTsFile, "[ERROR] prepareSinewaveForCPU() -> vkMapMemory() failed\n");
            return(vkResult);
    ...
    {
        vkResult = vkMapMemory(vkDevice, vertex_position_64x64_graphics.vkDeviceMemory, 0, size, 0, &data);
        if(vkResult != VK_SUCCESS)
        {
            fprintf(gpTsFile, "[ERROR] prepareSinewaveForCPU() -> vkMapMemory() failed\n");
            return(vkResult);
 ...
 void fillSinewaveArraysForCPU(unsigned int meshWidth, unsigned int meshHeight, float animTime)
 {
    // Code
    for(int i = 0; i < (int)meshWidth; i++)
    {
        for(int j = 0; j < (int)meshHeight; j++)
        {
            for(int k =0; k < 4; k++)
            {
                ...
                float w = sinf(u * frequency + animTime) * cosf(v * frequency + animTime) * 0.5f;
