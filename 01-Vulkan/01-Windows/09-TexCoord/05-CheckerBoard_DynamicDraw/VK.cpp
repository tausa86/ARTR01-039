...
float rectanglePosition[] = {
    ...
    float rectanglePosition_45_degree[] = {
        // First Triangle
        2.41421f, 1.0f, -1.41421f,    // Top Right
...
VkDescriptorSet vkDescriptorSet_straight = VK_NULL_HANDLE;   // Descriptor Set
...
// For checkerboard texture
BYTE checkerBoard[CHECKERBOARD_HEIGHT][CHECKERBOARD_WIDTH][4]; // 4 for RGBA
...
VkResult TsInitialize(void)
{
              ...
              // Create Vulkan (Logical) device
               ...
               // Texture creation
               vkTsResult = TsCreateTexture();
               if (VK_SUCCESS != vkTsResult)
               {
                    fflush(gpTsFile);
                    fprintf(gpTsFile, "[ERROR] TsInitialize() -> TsCreateTexture() failed with %d at %d\n", vkTsResult, __LINE__);
                    return(vkTsResult);
               }
               else
               {
                    fprintf(gpTsFile, "[INFO] TsInitialize() -> TsCreateTexture() succeeded at %d \n", __LINE__);
               }
...
void TsUninitialize(void)
{
               ...
               if (vkDevice)
               {
                              ...
                              if(uniformData_straight.vkBuffer)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkDestroyBuffer() is successfully done for uniformData_straight.vkBuffer\n");
                              }
                              ...
                              if(uniformData_straight.vkDeviceMemory)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkFreeMemory() is successfully done for uniformData_straight.vkDeviceMemory\n");
                              }
...
VkResult TsCreateVertexBuffer(void)
{
    ...
    vkBufferCreateInfo.size = sizeof(rectanglePosition);
    vkBufferCreateInfo.usage = VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    ...
    // Step 12
    //memcpy(data, rectanglePosition, sizeof(rectanglePosition));
    ...
    vkBufferCreateInfo.size = sizeof(rectangleTexcoord);
    ...
    // Step 12
    memcpy(data, rectangleTexcoord, sizeof(rectangleTexcoord));
...
{
    // Function declaration
    void TsMakeCheck(void);

    // Local variable declaration
    VkResult vkTsResult = VK_SUCCESS;

    // Code
    // Step-1 Get image info using functions
    TsMakeCheck(); // Create checkboard pattern if image is not found

    uint8_t * image_data = NULL;
    int texture_width, texture_height;

    image_data = (uint8_t *)checkerBoard;
    texture_width = CHECKERBOARD_WIDTH;
    texture_height = CHECKERBOARD_HEIGHT;
    ...
    // As copying of image data is already done into staging buffer, we can free the actual image data
    image_data = NULL;
    ...
    vkSamplerCreateInfo.magFilter = VK_FILTER_NEAREST;  //VK_FILTER_LINEAR;
    vkSamplerCreateInfo.minFilter = VK_FILTER_NEAREST;  //VK_FILTER_LINEAR;
...
// Create Staging Buffer for Buffer Copy
VkResult TsCreateStagingBuffer(float positionData[], size_t dataSize)
{
    ...
    // Code
    // Step-2 create staging buffer for image data
    // Step-1 Staging Buffer Variable
    ...
    vkBufferCreateInfo_stagingBuffer.flags = 0; // Valid flags used in scattered/sparse buffer

    vkBufferCreateInfo_stagingBuffer.size = dataSize;
    vkBufferCreateInfo_stagingBuffer.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;  // Data Transfer Source Buffer i.e. Vulkan is data centric
    ...
    // & use them repeatetively for different resources
    // sharingMode = 1 then other 2 members need to fill with queue family index and queue family array
    vkBufferCreateInfo_stagingBuffer.sharingMode = VK_SHARING_MODE_EXCLUSIVE;   // Can be used as multi-threading

    vkTsResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo_stagingBuffer, NULL, &vertexData_stagingBuffer_position.vkBuffer);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateStagingBuffer() -> [Staging Buffer] vkCreateBuffer() failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> [Staging Buffer] vkCreateBuffer() succeeded at %d \n", __LINE__);
    }
    ...
    for(uint32_t i = 0; i < vkPhysicalDeviceMemoryProperties.memoryTypeCount; i++)
    {
        if(1 == (vkMemoryRequirements_stagingBuffer.memoryTypeBits & 1))
        {
    ...
    vkTsResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo_stagingBuffer, NULL, &vertexData_stagingBuffer_position.vkDeviceMemory);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateStagingBuffer() -> [Staging Buffer] vkAllocateMemory() failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> [Staging Buffer] vkAllocateMemory() succeeded at %d \n", __LINE__);
    }

    // It binds vulkan device buffer object handle with vulkan device memory object handle
    vkTsResult = vkBindBufferMemory(vkDevice, vertexData_stagingBuffer_position.vkBuffer, vertexData_stagingBuffer_position.vkDeviceMemory, 0);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateStagingBuffer() -> [Staging Buffer] vkBindBufferMemory() failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> [Staging Buffer] vkBindBufferMemory() succeeded at %d \n", __LINE__);
    }

    void * data = NULL;

    vkTsResult = vkMapMemory(vkDevice, vertexData_stagingBuffer_position.vkDeviceMemory, 0, vkMemoryAllocateInfo_stagingBuffer.allocationSize, 0, &data);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateStagingBuffer() -> [Staging Buffer] vkMapMemory() failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> [Staging Buffer] vkMapMemory() vkMemoryAllocateInfo_stagingBuffer.allocationSize: %ld size_of: %ld succeeded at %d \n",(unsigned long)vkMemoryAllocateInfo_stagingBuffer.allocationSize, (unsigned long)sizeof(rectanglePosition), __LINE__);
    }

    // Host function
    memcpy(data, positionData, dataSize);
    ...
    vkTsResult = vkAllocateCommandBuffers(vkDevice, &vkCommandBufferAllocateInfo, &vkCommandBuffer);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateStagingBuffer() -> vkAllocateCommandBuffers() failed to 'Command Copy' at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> vkAllocateCommandBuffers() succeeded to 'Command Copy' at %d \n", __LINE__);
    }
    ...
    vkCommandBufferBeginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;   // Reset is not required as one time only

    vkTsResult = vkBeginCommandBuffer(vkCommandBuffer, &vkCommandBufferBeginInfo);
    if(VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateStagingBuffer() -> vkBeginCommandBuffer() for vkCommandBuffer failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> vkBeginCommandBuffer() for vkCommandBuffer succeeded at %d \n", __LINE__);
    }
    ...
    vkBufferMemoryBarrier.srcAccessMask = VK_ACCESS_SHADER_READ_BIT;  // Memory access stage in pipeline
    vkBufferMemoryBarrier.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT; // Memory access stage in pipeline
    ...
    vkBufferMemoryBarrier.size = dataSize;

    vkCmdPipelineBarrier(vkCommandBuffer, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 1, &vkBufferMemoryBarrier, 0, NULL);
    */
    VkBufferCopy vkBufferCopy;
    memset((void *)&vkBufferCopy, 0, sizeof(vkBufferCopy));

    vkBufferCopy.srcOffset = 0;
    vkBufferCopy.dstOffset = 0; // Need entire buffer thus SRC and DST both are 0
    vkBufferCopy.size = dataSize;
    ...
    // Because we are copying from staging buffer to vertex buffer
    /*memset((void *)&vkBufferMemoryBarrier, 0, sizeof(VkBufferMemoryBarrier));   
    ...
    vkCmdPipelineBarrier(vkCommandBuffer, 
        ...
        0, 0, NULL, 1, &vkBufferMemoryBarrier, 0, NULL);*/

    vkTsResult = vkEndCommandBuffer(vkCommandBuffer);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateStagingBuffer() -> [Buffer Copy] vkEndCommandBuffer() failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> [Buffer Copy] vkEndCommandBuffer() succeeded at %d \n", __LINE__);
    }
    ...
    // Intercommand buffer synchronization is not there due to 1 buffer thus pWaitDstStageMask is not required

    vkTsResult = vkQueueSubmit(vkQueue, 1, &vkSubmitInfo, VK_NULL_HANDLE);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateStagingBuffer() -> [Buffer Copy] vkQueueSubmit() failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> [Buffer Copy] vkQueueSubmit() succeeded at %d \n", __LINE__);
    }

    // Wait till queue's work is done
    vkTsResult = vkQueueWaitIdle(vkQueue);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateStagingBuffer() -> [Buffer Copy] vkQueueWaitIdle() failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> [Buffer Copy] vkQueueWaitIdle() succeeded at %d \n", __LINE__);
    }
    ...
    if(vkCommandBuffer)
    {
        ...
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> [Buffer Copy] vkFreeCommandBuffers() succeeded at %d \n", __LINE__);
    }
    ...
    if(vertexData_stagingBuffer_position.vkDeviceMemory)
    {
        ...
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> [Buffer Copy] vkFreeMemory() vertexData_stagingBuffer_position.vkDeviceMemory succeeded at %d \n", __LINE__);
    }
    ...
    if(vertexData_stagingBuffer_position.vkBuffer)
    {
        ...
        fprintf(gpTsFile, "[INFO] TsCreateStagingBuffer() -> [Buffer Copy] vkDestroyBuffer() vertexData_stagingBuffer_position.vkBuffer succeeded at %d \n", __LINE__);
    }
...
VkResult TsCreateUniformBuffer(void)
{
    ...
    // Step 6 straight
    memset((void *)&uniformData_straight, 0, sizeof(uniformData_straight));
    vkTsResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &uniformData_straight.vkBuffer);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateUniformBuffer() -> vkCreateBuffer() uniformData_straight failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateUniformBuffer() -> vkCreateBuffer() uniformData_straight succeeded at %d \n", __LINE__);
    }
    ...
    // Step 9
    vkTsResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &uniformData_straight.vkDeviceMemory);

    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateUniformBuffer() -> vkAllocateMemory() uniformData_straight failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateUniformBuffer() -> vkAllocateMemory() uniformData_straight succeeded at %d \n", __LINE__);
    } 

    // Step 10 it binds vulkan device buffer object handle with vulkan device memory object handle
    vkTsResult = vkBindBufferMemory(vkDevice, uniformData_straight.vkBuffer, uniformData_straight.vkDeviceMemory, 0);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateUniformBuffer() -> vkBindBufferMemory() uniformData_straight failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateUniformBuffer() -> vkBindBufferMemory() uniformData_straight succeeded at %d \n", __LINE__);
    }
...
VkResult TsUpdateUnifomBuffer(void)
{
    ...
    myuniformData.modelMatrix = translationMatrix; // Model Matrix = Translation Matrix
    ...
    vkTsResult = vkMapMemory(
        vkDevice,
        uniformData_straight.vkDeviceMemory, 
        0,
...
VkResult TsCreateDescriptorSet(void)
{
    ...
    // STRAIGHT Quad
    vkTsResult = vkAllocateDescriptorSets(vkDevice, &vkDescriptorSetAllocateInfo, &vkDescriptorSet_straight);
    if (VK_SUCCESS != vkTsResult)
    {
                    fprintf(gpTsFile, "[ERROR] TsCreateDescriptorSet() -> vkAllocateDescriptorSets() vkDescriptorSet_straight failed with %d at %d\n", vkTsResult, __LINE__);
                    return(vkTsResult);
    }
    else
    {
                    fprintf(gpTsFile, "[INFO] TsCreateDescriptorSet() -> vkAllocateDescriptorSets() vkDescriptorSet_straight succeeded at %d \n",  __LINE__);
    }
    ...
    fprintf(gpTsFile, "[INFO] vkUpdateDescriptorSets() vkDescriptorSet_straight is run successfully!!! at %d\n", __LINE__);
...
VkResult TsBuildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        TsCreateStagingBuffer(rectanglePosition, sizeof(rectanglePosition));

        // Bind our descriptor tset to the pipeline
        vkCmdBindDescriptorSets(vkCommandBuffer_array[i], VK_PIPELINE_BIND_POINT_GRAPHICS, vkPipelineLayout, 0, 1, &vkDescriptorSet_straight, 0, NULL);
        ...
        // Oblique Quad
        // Bind our descriptor tset to the pipeline
        TsCreateStagingBuffer(rectanglePosition_45_degree, sizeof(rectanglePosition_45_degree));
