...
VertexData vertexData_position_45_degree;
...
VkDescriptorSet vkDescriptorSet_oblique = VK_NULL_HANDLE;   // Descriptor Set
...
void TsUninitialize(void)
{
               ...
               if (vkDevice)
               {
                              ...
                              if(uniformData_oblique.vkBuffer)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkDestroyBuffer() is successfully done for uniformData_oblique.vkBuffer\n");
                              }
                              ...
                              if(uniformData_oblique.vkDeviceMemory)
                              {
                                ...
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkFreeMemory() is successfully done for uniformData_oblique.vkDeviceMemory\n");
                              }
                              ...
                              // [STEP-22]
                              if(vertexData_position_45_degree.vkDeviceMemory)
                              {
                                vkFreeMemory(vkDevice, vertexData_position_45_degree.vkDeviceMemory, NULL);
                                vertexData_position_45_degree.vkDeviceMemory = VK_NULL_HANDLE;
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkFreeMemory() vertexData_position_45_degree is successfully done\n");
                              }

                              if(vertexData_position_45_degree.vkBuffer)
                              {
                                vkDestroyBuffer(vkDevice, vertexData_position_45_degree.vkBuffer, NULL);
                                vertexData_position_45_degree.vkBuffer = VK_NULL_HANDLE;
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkDestroyBuffer() vertexData_position_45_degree is successfully done\n");
                              }
...
VkResult TsCreateVertexBuffer(void)
{
    ...
    // Vertex Position Buffer 45 degree
    // Step 4
    memset((void*)&vertexData_position_45_degree, 0, sizeof(VertexData));
    ...
    vkBufferCreateInfo.size = sizeof(rectanglePosition_45_degree);
    vkBufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    // sharingMode = 0 due to memset() which means Exclusive
    // In vulkan memory is not done in Bytes while it is done in Regions
    // and i.e. minimum it is 4096 & it is deliberatly made as Vulkan demands small number of large size allocations
    ...
    // Step 6
    vkTsResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_position_45_degree.vkBuffer);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkCreateBuffer() vertexData_position_45_degree failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkCreateBuffer() vertexData_position_45_degree succeeded at %d \n", __LINE__);
    }

    // Step 7
    memset((void*)&vkMemoryRequirements, 0, sizeof(VkMemoryRequirements));
    vkGetBufferMemoryRequirements(vkDevice, vertexData_position_45_degree.vkBuffer, &vkMemoryRequirements);
    // No error checking
    // Step 8
    memset((void*)&vkMemoryAllocateInfo, 0, sizeof(VkMemoryAllocateInfo));
    ...
    // Step 9
    vkTsResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_position_45_degree.vkDeviceMemory);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkAllocateMemory() vertexData_position_45_degree failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkAllocateMemory() vertexData_position_45_degree succeeded at %d \n", __LINE__);
    }
 
    // Step 10 it binds vulkan device buffer object handle with vulkan device memory object handle
    vkTsResult = vkBindBufferMemory(vkDevice, vertexData_position_45_degree.vkBuffer, vertexData_position_45_degree.vkDeviceMemory, 0);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkBindBufferMemory() vertexData_position_45_degree failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkBindBufferMemory() vertexData_position_45_degree succeeded at %d \n", __LINE__);
    }
    ...
    vkTsResult = vkMapMemory(
        vkDevice,
        vertexData_position_45_degree.vkDeviceMemory, //
        0, // start
        vkMemoryAllocateInfo.allocationSize, // sizeof
        0, // Reserved
        &data
    ...
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateVertexBuffer() -> vkMapMemory() vertexData_position_45_degree failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateVertexBuffer() -> vkMapMemory() vertexData_position_45_degree succeeded at %d \n", __LINE__);
    }

    // Step 12
    memcpy(data, rectanglePosition_45_degree, sizeof(rectanglePosition_45_degree));

    // Step 13
    vkUnmapMemory(vkDevice, vertexData_position_45_degree.vkDeviceMemory);
...
VkResult TsCreateUniformBuffer(void)
{
    ...
    vkTsResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &uniformData_oblique.vkBuffer);
    if (VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateUniformBuffer() -> vkCreateBuffer() uniformData_oblique failed at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateUniformBuffer() -> vkCreateBuffer() uniformData_oblique succeeded at %d \n", __LINE__);
    }
    ...
    // Step 9
    vkTsResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &uniformData_oblique.vkDeviceMemory);
    ...
    // Step 10 it binds vulkan device buffer object handle with vulkan device memory object handle
    vkTsResult = vkBindBufferMemory(vkDevice, uniformData_oblique.vkBuffer, uniformData_oblique.vkDeviceMemory, 0);
    if (VK_SUCCESS != vkTsResult)
    {
...
VkResult TsUpdateUnifomBuffer(void)
{
    ...
    rotationMatrix = glm::rotate(
        glm::mat4(1.0f),    // Identity Matrix Transform matrix
        glm::radians(30.0f),    // Angle in radians
    ...
    vkTsResult = vkMapMemory(
        vkDevice,
        uniformData_oblique.vkDeviceMemory, 
        0,
        sizeof(MyuniformData),
        0,
        &data
    ...
    if(VK_SUCCESS != vkTsResult)
    {
        fprintf(gpTsFile, "[ERROR] TsUpdateUnifomBuffer() -> vkMapMemory() uniformData_oblique Failed at %d\n", __LINE__);
        return(vkTsResult);
    }
...
VkResult TsCreateDescriptorSet(void)
{
    ...
    // OBLIQUE Quad
    vkTsResult = vkAllocateDescriptorSets(vkDevice, &vkDescriptorSetAllocateInfo, &vkDescriptorSet_oblique);
    if (VK_SUCCESS != vkTsResult)
    {
                    fprintf(gpTsFile, "[ERROR] TsCreateDescriptorSet() -> vkAllocateDescriptorSets() vkDescriptorSet_oblique failed with %d at %d\n", vkTsResult, __LINE__);
                    return(vkTsResult);
    }
    else
    {
                    fprintf(gpTsFile, "[INFO] TsCreateDescriptorSet() -> vkAllocateDescriptorSets() vkDescriptorSet_oblique succeeded at %d \n",  __LINE__);
    }
    ...
    fprintf(gpTsFile, "[INFO] vkUpdateDescriptorSets() vkDescriptorSet_oblique is run successfully!!! at %d\n", __LINE__);
...
VkResult TsBuildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        // Oblique Quad
        // Bind our descriptor tset to the pipeline
        vkCmdBindDescriptorSets(vkCommandBuffer_array[i], VK_PIPELINE_BIND_POINT_GRAPHICS, vkPipelineLayout, 0, 1, &vkDescriptorSet_oblique, 0, NULL);

        memset((void*)vkDeviceSize_offset_position, 0, sizeof(VkDeviceSize) * ARRAY_SIZE(vkDeviceSize_offset_position));

        vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vertexData_position_45_degree.vkBuffer, vkDeviceSize_offset_position);
