...
VkResult TsInitialize(void)
{
              ...
              // Create Vulkan (Logical) device
               ...
               // Texture creation
               vkTsResult = TsCreateTexture("Smiley.png");
               if (VK_SUCCESS != vkTsResult)
               {
                    fflush(gpTsFile);
                    fprintf(gpTsFile, "[ERROR] TsInitialize() -> TsCreateTexture() failed for Smiley.png with %d at %d\n", vkTsResult, __LINE__);
                    return(vkTsResult);
               }
               else
               {
                              fprintf(gpTsFile, "[INFO] TsInitialize() -> TsCreateTexture() succeeded for Smiley.png at %d \n", __LINE__);
               }
...
VkResult TsCreateTexture(const char * textureFileName)
{
    ...
    stbi_set_flip_vertically_on_load(1); // Flip the image vertically while loading
...
VkResult TsUpdateUnifomBuffer(void)
{
    ...
    myuniformData.modelMatrix = translationMatrix; // Model Matrix = Translation Matrix * Rotation Matrix
