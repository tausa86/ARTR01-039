...
{
    glm::mat4 mvpMatrix;
    glm::vec4 numberOfLineSegments;
    glm::vec4 numberOfLineStrips;
    glm::vec4 lineColor;
};

unsigned int uiNumberOfLineSegements = 1; // Initial line segments
...
VkShaderModule vkShaderModule_tessellation_control_shader = VK_NULL_HANDLE;
VkShaderModule vkShaderModule_tessellation_evaluation_shader = VK_NULL_HANDLE;
...
LRESULT CALLBACK TsWndProc(HWND hTsWnd, UINT uiTsMsg, WPARAM wTsParam, LPARAM lTsParam)
{
               ...
               case WM_KEYDOWN:
                              ...
                              case 0x66:    // case 'f'
                                             ...
                                             break;
                              case VK_UP:
                              if(uiNumberOfLineSegements < 30)
                                uiNumberOfLineSegements += 1;
                            break;
                            case VK_DOWN:
                            if(uiNumberOfLineSegements > 1)
                                uiNumberOfLineSegements -= 1;
                            break;
                              default:
                              break;
...
void TsUninitialize(void)
{
               ...
               if (vkDevice)
               {
                              ...
                              if(vkShaderModule_tessellation_evaluation_shader)
                              {
                                vkDestroyShaderModule(vkDevice, vkShaderModule_tessellation_evaluation_shader, NULL);
                                vkShaderModule_tessellation_evaluation_shader = VK_NULL_HANDLE;
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkDestroyShaderModule() vkShaderModule_tessellation_evaluation_shader is successfully done\n");
                              }

                              if(vkShaderModule_tessellation_control_shader)
                              {
                                vkDestroyShaderModule(vkDevice, vkShaderModule_tessellation_control_shader, NULL);
                                vkShaderModule_tessellation_control_shader = VK_NULL_HANDLE;
                                fprintf(gpTsFile, "[INFO] TsUninitialize() -> vkDestroyShaderModule() vkShaderModule_tessellation_control_shader is successfully done\n");
                              }
...
VkResult TsCreateVulkanDevice(void)
{
              // Function declaration
               ...
               else
               {
                    ...
                    fprintf(gpTsFile, "[ERROR] TsCreateVulkanDevice() -> Tessellation Shader not supported with %d at %d\n", vkTsResult, __LINE__);
...
VkResult TsCreateVertexBuffer(void)
{
    ...
    vkBufferCreateInfo.size = sizeof(line_position);
    ...
    // Step 12
    memcpy(data, line_position, sizeof(line_position));
...
VkResult TsUpdateUnifomBuffer(void)
{
    ...
    modelMatrix = glm::translate(
        ...
        glm::vec3(-1.0f, -1.0f, -10.0f)    // Translation Vector x, y, z
    );
    glm::mat4 viewMatrix = glm::mat4(1.0);  // Identity Matrix
    glm::mat4 projectionMatrix = glm::mat4(1.0);    // Identity Matrix
    ...
    myuniformData.mvpMatrix = projectionMatrix * viewMatrix * modelMatrix;

    // Update line segments
    myuniformData.numberOfLineSegments.x = (float)uiNumberOfLineSegements;

    // Update line of strips
    myuniformData.numberOfLineStrips.x = 1.0; // 1 Line Strip

    // Update line color
    myuniformData.lineColor = glm::vec4(1.0, 1.0, 0.0, 1.0);    // Yellow Color

    TCHAR str[255];
    wsprintf(str, "TsVulkan Segment: %d", uiNumberOfLineSegements);
    SetWindowText(ghTsWnd, str);
...
VkResult TsCreateShaders(void)
{
    ...
    if(NULL == fp)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> tesc Failed to open %s at %d\n", szFileName, __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> tesc Succeeded to open %s at %d\n", szFileName, __LINE__);
    }
    ...
    if(0 == size)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> tesc The SPIRV shader file size 0 %s at %d\n", szFileName, __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkTsResult);
    }
    ...
    if(1 != retVal)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> tesc Failed to read the SPIRV shader file %s at %d\n", szFileName, __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> tesc Succeeded to read %s at %d\n", szFileName, __LINE__);
    }
    ...
    vkTsResult = vkCreateShaderModule(vkDevice, &vkShaderModuleCreateInfo, NULL, &vkShaderModule_tessellation_control_shader);
    if(NULL == fp)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> tesc vkCreateShaderModule() Failed in tesselation control shader at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> tesc vkCreateShaderModule() Succeeded in tesselation control shader at %d\n", __LINE__);
    }
    ...
    fprintf(gpTsFile, "[INFO] TsCreateShaders() -> Tessellation Control Shader Module is successfully created!!!\n");
    ...
    if(NULL == fp)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> tese Failed to open %s at %d\n", szFileName, __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> tese Succeeded to open %s at %d\n", szFileName, __LINE__);
    }
    ...
    if(0 == size)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> tese The SPIRV shader file size 0 %s at %d\n", szFileName, __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkTsResult);
    }
    ...
    if(1 != retVal)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> tese Failed to read the SPIRV shader file %s at %d\n", szFileName, __LINE__);
        vkTsResult = VK_ERROR_INITIALIZATION_FAILED;
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> tese Succeeded to read %s at %d\n", szFileName, __LINE__);
    }
    ...
    vkTsResult = vkCreateShaderModule(vkDevice, &vkShaderModuleCreateInfo, NULL, &vkShaderModule_tessellation_evaluation_shader);
    if(NULL == fp)
    {
        fprintf(gpTsFile, "[ERROR] TsCreateShaders() -> vkCreateShaderModule() Failed in tessellation Evaluation shader at %d\n", __LINE__);
        return(vkTsResult);
    }
    else
    {
        fprintf(gpTsFile, "[INFO] TsCreateShaders() -> vkCreateShaderModule() Succeeded in tessellation Evaluation shader at %d\n", __LINE__);
    }
    ...
    fprintf(gpTsFile, "[INFO] TsCreateShaders() -> Tessellation Evaluation Shader Module is successfully created!!!\n");
...
VkResult TsCreateDescriptorSetLayout(void)
{
    ...
    vkDescriptorSetLayoutBinding.stageFlags = VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT | VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
...
VkResult TsCreatePipeline(void)
{
    ...
    vkVertexInputBindingDescription_array[0].stride = sizeof(float) * 2;    // because geometry is line thus only needs x, y and so 2
    ...
    vkVertexInputAttributeDescription_array[0].format = VK_FORMAT_R32G32_SFLOAT;     // Majha vertex X Y aahe te Signed Float 32-bit R32 = X, G32 = Y
    ...
    VkPipelineShaderStageCreateInfo vkPipelineShaderStageCreateInfo_array[4];   // Vertex and Fragment shaders
    ...
    vkPipelineShaderStageCreateInfo_array[1].module = vkShaderModule_tessellation_control_shader;
    ...
    vkPipelineShaderStageCreateInfo_array[2].pNext = NULL;  // jya jya structures na extensions aahet tya tya structure la pNext dena must aahe
    ...
    vkPipelineShaderStageCreateInfo_array[2].module = vkShaderModule_tessellation_evaluation_shader;
    vkPipelineShaderStageCreateInfo_array[2].pName = "main";    // Name of entry point function of vertex shader
    vkPipelineShaderStageCreateInfo_array[2].pSpecializationInfo = NULL;    // LLVM (Low Level Virtual Machine) precompiled binding of constants in SPIR-V

    // Fragment shader
    vkPipelineShaderStageCreateInfo_array[3].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    vkPipelineShaderStageCreateInfo_array[3].pNext = NULL;  // jya jya structures na extensions aahet tya tya structure la pNext dena must aahe
    ...
    vkPipelineShaderStageCreateInfo_array[3].module = vkShaderModule_fragment_shader;
    vkPipelineShaderStageCreateInfo_array[3].pName = "main";    // Name of entry point function of vertex shader
    vkPipelineShaderStageCreateInfo_array[3].pSpecializationInfo = NULL;    // LLVM (Low Level Virtual Machine) precompiled binding of constants in SPIR-V
    ...
    vkPipelineTessellationStateCreateInfo.patchControlPoints = 4;   // Similar to the 2nd parameter of glPatchParameteri() in RTR
...
VkResult TsBuildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCmdDraw(vkCommandBuffer_array[i],
            4,  // Kiti vertices draw karaychet
