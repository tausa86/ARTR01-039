...
VkResult TsUpdateUnifomBuffer(void)
{
    ...
    myuniformData.lightAmbient[0] = 1.0f;
    myuniformData.lightAmbient[1] = 1.0f;
    myuniformData.lightAmbient[2] = 1.0f;
    ...
    myuniformData.materialAmbient[0] = 1.0f;
    myuniformData.materialAmbient[1] = 1.0f;
    myuniformData.materialAmbient[2] = 1.0f;
...
VkResult TsBuildCommandBuffers(void)
{
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        if(vertexData_position.vkBuffer != VK_NULL_HANDLE)
            ...
            fprintf(gpTsFile, "[ERROR] TsBuildCommandBuffers() -> vertexData_position.vkBuffer is null failed for %d index with %d at %d\n", i, vkTsResult, __LINE__);
        ...
        if(vertexData_normal.vkBuffer != VK_NULL_HANDLE)
            vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 1, 1, &vertexData_normal.vkBuffer, vkDeviceSize_offset_normals);
        else
            fprintf(gpTsFile, "[ERROR] TsBuildCommandBuffers() -> vertexData_normal.vkBuffer is null failed for %d index with %d at %d\n", i, vkTsResult, __LINE__);
        ...
        if(vertexData_texcoord.vkBuffer != VK_NULL_HANDLE)
