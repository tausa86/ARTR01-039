...
{
    // Code
    angle = angle + 0.07f; // Increment angle for rotation
    if(angle >= 360.0f)
    {
        angle = 0.0f; // Reset angle to 0.0f
    }
}
...
VkResult TsUpdateUnifomBuffer(void)
{
    ...
    translationMatrix = glm::translate(
        ...
        glm::vec3(0.0f, 0.0f, -4.0f)    // Translation Vector x, y, z
    );
    ...
    rotationMatrix = glm::rotate (
        ...
        glm::vec3(1.0f, 0.0f, 0.0f)    // Across X-Axis rotation
    );
...
VkResult TsCreatePipeline(void)
{
    ...
    // Assign viewport to viewport state
    // This is used to map the viewport to the framebuffer
    ...
    // Assign scissor to viewport state
    // This is used to clip the pixels outside the viewport
