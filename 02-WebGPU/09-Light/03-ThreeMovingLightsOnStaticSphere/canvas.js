...
var lightPosition = new Float32Array([0.0, 0.0, 0.0, 1.0, // YZ Position
    0.0, 0.0, 0.0, 1.0, // XZ Position
...
var lightAngle_0 = 0.0;
var lightAngle_1 = 0.0;
var lightAngle_2 = 0.0;
...
function initialize() {
    ...
    "{\n" +
        ...
        "@location(2) lightDirection_0 : vec3<f32>,\n" +
        "@location(3) lightDirection_1 : vec3<f32>,\n" +
        "@location(4) lightDirection_2 : vec3<f32>\n" +
    "};\n" +    
    ...
    "{\n"+
        ...
        "let reflectionVector : vec3<f32> = reflect(-output.lightDirection_0, output.transformedNormal);\n" +
    ...
    const myUniformBufferSize = Float32Array.BYTES_PER_ELEMENT * 16 +   // Model Matrix: from 0th to 63rd byte offset
                                ...
                                // Light 1
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Light Ambient: from 256th to 271st byte offset
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Light Diffuse: from 272nd to 287th byte offset
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Light Specular: from 288th to 303rd byte offset
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Light Position: from 304th to 319th byte offset

                                // Light 2
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Light Ambient: from 320th to 335th byte offset
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Light Diffuse: from 336th to 351st byte offset
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Light Specular: from 352nd to 367th byte offset
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Light Position: from 368th to 383rd byte offset

                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Material Ambient: from 384th to 399th byte offset
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Material Diffuse: from 400th to 415th byte offset
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Material Specular: from 416th to 431st byte offset
                                Float32Array.BYTES_PER_ELEMENT * 4 +    // Material Shininess: from 432nd to 447th byte offset

                                Uint32Array.BYTES_PER_ELEMENT * 4;      // LKeyPressed: from 448th to 463rd byte offset
...
function keyDown(event) {
    ...
    switch(event.key){      // event.keyCode is deprecated, so using event.key instead and key is a string, so using string values instead of integer key codes
        ...
        case "L":
        case "l":
            //console.log("keyDown() L/l key is pressed\n");
